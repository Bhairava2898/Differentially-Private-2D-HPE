...
from datautils.humanartset import COCODataset
...
def evaluate(model, val_loader, val_dataset, criterion, config: TrainConfig, device):
    ...
    with torch.no_grad():
        for image, target_x, target_y, target_weight, meta in pit(val_loader, color="red"):
...
def train(config: TrainConfig) -> None:
    ...
    flops = FlopCountAnalysis(model, torch.randn(1, 3, config.image_height, config.image_width)).total()
    print(f"FLOPs: {flops}")
    ...
    for epoch in pit(range(config.epochs), color="green"):
        ...
        for image, target_x, target_y, target_weight, _ in pit(train_loader, color="blue"):
            image = image.to(device)
            target_x = target_x.to(device)
            target_y = target_y.to(device)
            target_weight = target_weight.to(device)
