...
class JointsDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        blur_affine = cv2.GaussianBlur(input,
                                       ksize=(25, 25),    # e.g. 15×15
                                       sigmaX=10)   

        if self.transform:
            input = self.transform(input)
            blur_input = self.transform(blur_affine)
        ...
        if self.coord_representation == 'simdr':
            ...
            target_weight = torch.from_numpy(target_weight)
            return input,blur_input, target_x, target_y, target_weight, meta            
