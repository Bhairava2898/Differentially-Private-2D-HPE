...
def cpu_nms(dets, thresh):
    ...
    returns: np.ndarray of kept indices
    """
    # convert boxes+scores to torch, run nms, return numpy indices
    dets_t = torch.tensor(dets, dtype=torch.float32)
    boxes  = dets_t[:, :4]
    scores = dets_t[:, 4]
    keep   = torch_nms(boxes, scores, thresh)
    return keep.cpu().numpy()

def gpu_nms(dets, thresh):
    """
    Same as cpu_nms, but assumes GPU is available.
    """
    dets_t = torch.tensor(dets, dtype=torch.float32, device='cuda')
    boxes  = dets_t[:, :4]
    scores = dets_t[:, 4]
    keep   = torch_nms(boxes, scores, thresh)
    return keep.cpu().numpy()
